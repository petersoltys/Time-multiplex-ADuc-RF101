...
char pktMemory[4][10][255];



signed char numOfPkt [2][4] = {{-1,-1,-1,-1},{-1,-1,-1,-1}};
...
void uartInit(void){
	UrtLinCfg(0,128000,(0x3 << 0 ),(0x0 << 2 ));
  DioCfg(((ADI_GPIO_TypeDef *)0x40006030UL),0x9); 
  
  DmaInit();
  DmaTransferSetup(2,  20,   Buffer);
...
void ledInit(void){
  ...
  DioCfg(((ADI_GPIO_TypeDef *)0x400060C0UL),0x10);
  DioOen(((ADI_GPIO_TypeDef *)0x400060C0UL), 4); 
}
...
void blink(unsigned int time){
  GptLd (((ADI_TIMER_TypeDef *)0x40000400UL), time); 
  GptCfg(((ADI_TIMER_TypeDef *)0x40000400UL), (0x2 << 5 ), (0x3 << 0 ), (0x1 << 4 )|(0x1 << 3 ));
  while (GptSta(((ADI_TIMER_TypeDef *)0x40000400UL))& (0x1 << 6 )); 
  GptClrInt(((ADI_TIMER_TypeDef *)0x40000400UL),(0x1 << 0 ));
  while (GptSta(((ADI_TIMER_TypeDef *)0x40000400UL))& (0x1 << 7 )); 
  NVIC_EnableIRQ(TIMER1_IRQn);
}
void blinkDisable(){
	NVIC_DisableIRQ(TIMER1_IRQn);
	((ADI_GPIO_TypeDef *)0x400060C0UL)->GPSET |= 0x04;
}
...
void radioInit(void){
  ...
  if (RIE_Response == RIE_Success)
     RIE_Response = RadioSetFrequency(433920000);
...
void timeMultiplexInit(){
  GptLd (((ADI_TIMER_TypeDef *)0x40000000UL), 1270); 
  GptCfg(((ADI_TIMER_TypeDef *)0x40000000UL), (0x0 << 5 ), (0x2 << 0 ), (0x1 << 4 )|(0x1 << 3 ));
  while (GptSta(((ADI_TIMER_TypeDef *)0x40000000UL))& (0x1 << 6 )); 
  GptClrInt(((ADI_TIMER_TypeDef *)0x40000000UL),(0x1 << 0 ));
  while (GptSta(((ADI_TIMER_TypeDef *)0x40000000UL))& (0x1 << 7 )); 
  ...
  for (S=0;S<4;S++)
    {
      ...
      if (S==2)
        DioClr(((ADI_GPIO_TypeDef *)0x400060C0UL),4);
      ...
      while(str[k] != '\0')
      {
        ...
        strcpy(&pktMemory[(Buffer[1]-'0')][(Buffer[3]-'0')][0],Buffer) ;
        k++;                                                                                                              
...
void RadioRecieve(void){
  ...
  DioSet(((ADI_GPIO_TypeDef *)0x400060C0UL),4);
	if (RIE_Response == RIE_Success)
		 {
       while (!RadioRxPacketAvailable()){
         if (send == 1){
           ...
           if (SID < 4){
             rf_printf("Slave %d Time Slot",SID);
...
{ 
  ...
  NVIC_SetPriority(DMA_UART_TX_IRQn,7);
  NVIC_SetPriority(TIMER1_IRQn,8);
  NVIC_SetPriority(TIMER0_IRQn,9);
  NVIC_SetPriority(EINT8_IRQn,10);
	...
	while(1)
  {
    ...
    if (firstRxPkt==0)
      {
      firstRxPkt=1;
      numOfPkt[actualRxBuffer][slv] = Buffer[5]-'0';
    ...
    strcpy(&pktMemory[slv][pkt][0],Buffer) ;
    nextRxPkt++;
  }
}
...
void GP_Tmr1_Int_Handler (void)
{
  ...
  if (GptSta(((ADI_TIMER_TypeDef *)0x40000400UL))== (0x1 << 0 )) 
  {
    DioTgl(((ADI_GPIO_TypeDef *)0x400060C0UL),4); 
...
void GP_Tmr0_Int_Handler(void){
  if (GptSta(((ADI_TIMER_TypeDef *)0x40000000UL))== (0x1 << 0 )) 
  {
    nextRxPkt = 0;
    firstRxPkt = 0;
    terminate_flag=1;
...
void DMA_UART_TX_Int_Handler (void)
{
  ...
  if(empty_flag == 1){
    if(dmaTxSlv < 4){
      if(dmaTxPkt <= numOfPkt[actualTxBuffer][dmaTxSlv]){
        
        dmaTxPtr=&pktMemory[dmaTxSlv][dmaTxPkt][0];
        
        if(dmaTxPtr[3]!='w'){
          while(dmaTxPtr[len]!='\0'){
            len++;
          }

          dmaSend(dmaTxPtr,len);
          ...
          dma_printf("missing packet %d of slave %d ",dmaTxPkt,dmaTxSlv);
          dmaTx_flag = 1 ;
        }
        dmaTxPkt++;
      }
