...
#define	CS_LOW()	DioSet(pADI_GP0,DioRd(pADI_GP0)|0x08)		/* Set CS Low */
#define	CS_HIGH()	DioSet(pADI_GP0,DioRd(pADI_GP0)&0xf7)		/* Set CS High */
#define	INS			1//(!PORT0.PORT.BIT.B7)	/* Card status switch (Exist:True, Empty:False) */
...
void power_on (void)
{
  DioCfg(pADI_GP0,GP0CON_CON0_SPI1MISO | GP0CON_CON1_SPI1SCLK | GP0CON_CON2_SPI1MOSI | GP0CON_CON3_SPI1CS0); 
  SpiBaud(pADI_SPI1,0x3F,SPIDIV_BCRST_EN);
...
{
	SpiTx(pADI_SPI1, dat);		            // transmit command or any dummy data
	while ((SpiSta(pADI_SPI1) & SPISTA_TX) != SPISTA_TX) ; // wait for data received status bit
	return SpiRx(pADI_SPI1);
}
...
)
{
	int i = 0;
	  for (i=0;i<btx;i++)
  {
    SpiTx(pADI_SPI1, buff[i]);		            // transmit command or any dummy data
    while ((SpiSta(pADI_SPI1) & SPISTA_TX) != SPISTA_TX) ; // wait for data received status bit
    SpiRx(pADI_SPI1);
  } 
...
)
{
	int i = 0;
	  for (i=0;i<btr;i++)
  {
    SpiTx(pADI_SPI1, 0xff);		            // transmit command or any dummy data
    while ((SpiSta(pADI_SPI1) & SPISTA_TX) != SPISTA_TX) ; // wait for data received status bit
    buff[i] = SpiRx(pADI_SPI1);
  } 
// 	DWORD *lp = (DWORD*)buff;
